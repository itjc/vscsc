...
void VssClient::GenerateSetvarScript(wstring stringFileName)
{
    ...
    ofile << L"@echo.\n";
    ofile << L"@echo [This script is generated by VSCSC.EXE for the shadow set " << Guid2WString(m_latestSnapshotSetID).c_str() << L"]\n";
    ofile << L"@echo.\n\n";
...
// This is useful for management operations
wstring VssClient::GetLastShadowDeviceName()
{
     FunctionTracer ft(DBG_INFO);
    // For each added volume add the VSCSC.EXE exposure command
    for (unsigned i = 0; i < m_latestSnapshotIdList.size(); i++)
    {
        ...
        if ((m_dwContext & VSS_VOLSNAP_ATTR_TRANSPORTABLE) == 0)
        {
            ...
            return wstring(Snap.m_pwszSnapshotDeviceObject);
        }
    }
	return L"";
}
